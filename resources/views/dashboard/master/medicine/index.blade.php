  ...
  <x-card.table id="medicine" :columns="['Nama', 'Satuan', 'Jenis', 'Kategori']" create-button-type="link" :create-route="route('master.medicine.create')">
    @foreach ($medicines as $medicine)
      <x-table.row :$loop :id="$medicine->uuid"
        :edit-route="route('master.medicine.edit', $medicine->uuid)"
        :detail-route="route('master.medicine.show', $medicine->uuid)" 
        :delete-route="route('master.medicine.destroy', $medicine->uuid)"
        :no-delete-confirm="true"
      >
        <td>{{ $medicine->name }}</td>
